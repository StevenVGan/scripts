...
BASE="${HOME}/seq/CUTRUN/240529_CnR_ER_SIM-A9_Steven"
...
GENES_DN=3000
BODY_LENGTH=7500
GENOME_BED="${HOME}/ref/annot/mm10/mm10_annot_genome.bed"
CPU=8
COLOR="RdBu"
SORT="descend"
...
LOGFILE="$OUTPUT_DIR/5_heatmap_$(date +%Y%m%d_%H%M%S).log"
...
#  SORT="no"
  ...
  nice $DEEPTOOLS/computeMatrix reference-point --referencePoint "center" -b $PEAK_UP -a $PEAK_DN \
    ...
    --missingDataAsZero -p $CPU --samplesLabel $bw_labelss
...
for mtx_name in "${mtx_names[@]}"; do
  ...
  nice $DEEPTOOLS/plotHeatmap -m $mtx_file -out $plot_out --colorMap $COLOR \
    --zMin -8 --zMax 8 --missingDataColor 1 $REGIONSLABEL --sortRegions $SORT
