...
#      (run_spp.R) is intentionally omitted — NSC/RSC use a ChIP-style fragment-
#      bimodal cross-correlation signature that does not fit TSS-focused csRNA.
#   2) deepTools: plotFingerprint (--ignoreDuplicates enabled for PE only —
#      deepTools' PE duplicate criterion uses both mate positions, so biological
#      TSS stacks are preserved and only fragment-identical PCR duplicates get
#      collapsed; SE dedup is 5′-only and would flatten TSS signal, so the flag
#      is omitted when SE=1), multiBamSummary → plotPCA / plotCorrelation.
#   3) MACS3 summary (optional): only if MACS3_DIR has peak files. 4.3 TSS
#      summary (optional): only if TSS_DIR has findcsRNATSS outputs.
#   4) Write multiqc_config.yaml (merges sample names; order/visibility tuned
#      for csRNA modules).
#   5) Run multiqc
###############################################################################
...
# plotFingerprint — --ignoreDuplicates for PE only. deepTools PE dedup uses BOTH
# mate positions (fragment coordinates), so csRNA biological stacks at a shared
# 5′ TSS with varying 3′ fragment ends are preserved; only fragment-identical
# PCR duplicates are collapsed. SE dedup is 5′-only and would flatten TSS signal.
fp_extra=()
...
###############################################################################
# STEP 3b: csRNA TSS summary (optional — only if TSS_DIR has findcsRNATSS output)
###############################################################################

TSS_SUMMARY="${TSS_DIR}/custom_tss_stats_mqc.tsv"
tss_has_output=0
for f in "${TSS_DIR}"/*/*.tss.txt; do
  [[ -f "$f" ]] && { tss_has_output=1; break; }
done

if [[ "$tss_has_output" -eq 1 ]]; then
  echo "=== STEP 3b: csRNA TSS summary ==="
  echo -e "Sample\tTSS_clusters" > "$TSS_SUMMARY"
  for f in "${TSS_DIR}"/*/*.tss.txt; do
    [[ -f "$f" ]] || continue
    name=$(basename "$f" .tss.txt)
    # HOMER peak files start with '#' comment lines; count data rows only
    n_tss=$(grep -c -v '^#' "$f" 2>/dev/null || echo 0)
    echo -e "${name}\t${n_tss}" >> "$TSS_SUMMARY"
  done
else
  echo "=== STEP 3b: TSS_DIR empty or no *.tss.txt; skipping csRNA TSS summary ==="
fi
...
# Build config: base + optional MACS / TSS custom_data blocks
CUSTOM_DATA_BLOCK=""
...
if [[ -f "$TSS_SUMMARY" && -s "$TSS_SUMMARY" ]]; then
  CUSTOM_DATA_BLOCK+="
  custom_tss_stats:
    file: \"tss/custom_tss_stats_mqc.tsv\"
    section_name: \"csRNA TSS Summary (findcsRNATSS)\"
    plot_type: \"table\"
    pconfig:
      id: \"custom_tss_stats\"
      title: \"csRNA TSS Cluster Counts\"
"
fi
CUSTOM_BLOCK=""
