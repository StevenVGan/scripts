...
#  R2="${R1%R1.fastq.gz}R2.fastq.gz"
  ...
  nice $TRIM_GALORE --output_dir $OUTPUT_DIR -q 25 --phred33 -j $CORES \
                    --stringency $STRINGENCY --path_to_cutadapt $CUTADAPT --gzip $R1

done
COMMENT
echo "Running FastQC for all samples"
# Run FastQC with the specified parameters
nice $FASTQC --outdir ${OUTPUT_DIR}/fastqc --threads $CPU ${OUTPUT_DIR}/*.fq.gz
