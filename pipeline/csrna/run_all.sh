...
#     1) Trim Galore (auto 3′ adapter, --nextseq polyG) → cutadapt (R2 5′-adapter
#        readthrough, PE only) → FastQC → MultiQC
#     2) bowtie2 alignment + BAM/track generation
#     3) HOMER tag directories
#     4.1) MACS3 peak calling (4.1_peak_macs3.sh) — ChIP-style; off by default for csRNA
#     4.2) HOMER peak calling + annotation + blacklist filter (4.2_peak_homer.sh)
#     4.3) HOMER findcsRNATSS.pl — csRNA-specific TSS calling (4.3_tss_csrna.sh)
#     5) QC + MultiQC (5_qc.sh: preseq, plotFingerprint, PCA/Correlation, optional MACS3/TSS summaries)
#
# Configuration
...
echo "[INFO] BASE: $BASE"
echo "[INFO] Toggles: TRIM=$RUN_TRIM BOWTIE2=$RUN_BOWTIE2 TAGS=$RUN_HOMER_TAGS MACS3=$RUN_PEAK_MACS3 HOMER_PEAK=$RUN_PEAK_HOMER TSS_CSRNA=$RUN_TSS_CSRNA QC=$RUN_QC"
...
if [[ "$RUN_TSS_CSRNA" -eq 1 ]]; then
  bash "${SCRIPT_DIR}/4.3_tss_csrna.sh"
else
  echo "=== Skipping STEP 4.3 (RUN_TSS_CSRNA=0) ==="
fi
