...
- <e.g. "v3.3_NABEC dropped 14% of cells at QC — need Steven's call on filter strictness">
```
...
when it ceased to be microglia-specific. Consumers import via
...
```
analysis/
├── microglia_aging_v3/          # canonical
├── microglia_aging_v4/          # generalization test (kept; weaker but informative)
├── microglia_aging_v5/          # ML feature ranking (kept; complementary)
├── microglia_aging_v3.3_NABEC/  # cohort split of v3.3
└── microglia_esr1_v3/           # supersedes v2 (see README)
```

- `<topic>_vN` for major versions. `<topic>_vN_M` (`v3_5`, `v1.1_pooled`)
  for incremental tweaks within a version line.
- Suffix sub-variants with a context tag: `microglia_aging_v3.3_NABEC/`
  vs `microglia_aging_v3.3/` (cohort split), not `_v3.3a/_v3.3b/`.
...
```bash
# analysis/microglia_TFatlas_programs_v2/script/00_config.sh
export FOOTPRINT_TSV="${FOOTPRINT_TSV:-$PROJ_ROOT/../microglia_footprint_v2/processed/footprints.tsv}"
```

Lets the consumer flip between producer versions (`microglia_footprint/`
...
- **`Micro1` / `Micro2`** (not `M1` / `M2`) — `M1`/`M2` are the macrophage
  polarization vocabulary, a different concept; reusing them in microglia
  figures, obs columns, and writeups confuses readers.
- **`Mac_resTAM` / `Mac_monoTAM`** for peripheral tumor macrophages
  (both FOLR2+ and TREM2+ are TAMs per Helft); supersedes earlier
  "resident-vs-monocyte" framing.
- Monocyte-derived microglia retain canonical microglia markers
  (TMEM119/P2RY12/CX3CR1/MERTK transfer to engrafted monocytes within
  weeks); do **not** use those markers to rule out monocyte origin. Use
  Sall1/Itgal/Mef2c/CD44 etc. instead.
...
- **Dotplots default to absolute scale.** `sc.pl.dotplot(standard_scale=None, cmap="Reds")`.
  ...
  (P2RY12 etc.) look "hollow" because 1.1× differences get stretched to
  full-range contrast.
