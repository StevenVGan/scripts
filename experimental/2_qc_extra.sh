...
###############################################################################
# STEP 3: MACS3 summary + FRiP + FragLen
###############################################################################

echo "=== STEP 3: MACS3 summary + FRiP + Fragment Length ==="

# We rename the file slightly to avoid 'macs3' keyword collisions in auto-detection
MACS3_SUMMARY="${MACS3_DIR}/custom_macs3_stats_mqc.tsv"

# Header with Capitalized names
echo -e "Sample\tNum_Peaks\tPeak_Type\tFRiP\tFrag_Len" > "$MACS3_SUMMARY"
...
for bed in "${MACS3_DIR}"/*_filtered.bed; do
  ...
  echo -e "${name}\t${n_peaks}\t${peak_type}\t${frip}\t${frag_len}" >> "$MACS3_SUMMARY"
done
shopt -u nullglob
...
table_columns_visible:
  ...
  # --- MACS3 (Standard Module) ---
  # REQUEST: Only show Fragment Length (d) and Peak Count
  macs3-peak_count: True
  macs3-d: True
  
  # HIDE these MACS3 columns
  macs3-treatment_redundant_rate: False
  macs3-control_redundant_rate: False
  macs3-num_peaks: False   # Just in case of variable naming conflicts
...
# 6. CUSTOM DATA (Still keeping the Custom MACS3 table for the Detail section)
custom_data:
  custom_macs3_stats:
    file: "peaks/macs3/custom_macs3_stats_mqc.tsv"
    section_name: "MACS3 Peak Summary"
    description: "Detailed peak stats including FRiP (Fraction of Reads in Peaks)"
    plot_type: "table"
    pconfig:
      id: "custom_macs3_stats"
      title: "MACS3 Peak Metrics"
