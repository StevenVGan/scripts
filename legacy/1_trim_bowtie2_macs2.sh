...
# (trimming → QC → alignment → BAM processing → HOMER tags → MACS2 peaks + FRiP)
#
...
# - MACS2 groups:
#     defined in macs2_groups.tsv (see format in CONFIG)
# - This script:
...
#     * Calls MACS2 (narrow vs broad based on TF/Histone)
#     * Computes FRiP per MACS2 group into peaks/frip_scores.tsv
###############################################################################

############################ CONFIG SECTION ###################################

# TODO: This code is temporarly using macs3 via 'conda run -n macs3 macs3 ...' instead of macs2 directly on PATH. This is because of a zerodivision bug of macs2. Need to fix this properly later. 

# TODO: I have update to macs3, no need to keep the old macs2 conda env. Later we can just use macs3 directly on PATH.
...
BASE="${HOME}/work/seq/CnT/251215_CnT_H4K20me1_H3K27me3_MCF7_Steven"
...
MACS2_GROUPS_FILE="${BASE}/macs2_groups.tsv"
...
MACS2_FORMAT="BAMPE"
MACS2_FDR=0.05
MACS2_BROAD_CUTOFF=0.10            # used for Histone samples when calling broad peaks
...
for c in trim_galore cutadapt fastqc bowtie2 samtools bamCoverage \
         multiBamSummary macs2 bedtools makeTagDirectory; do
  check_cmd "$c"
done

# Create directories
mkdir -p "$TRIM_DIR" "$ALIGN_DIR" "$BAM_DIR" "$TRACK_DIR" "$BAMQC_DIR" "$TAG_DIR" "$PEAK_DIR" "$MACS2_DIR" "$LOG_DIR"

# Global log file
PIPELINE_LOG="${LOG_DIR}/1_trim_bowtie2_macs2_$(date +%Y%m%d_%H%M%S).log"
...
###############################################################################
# STEP 3: MACS2 peaks (flexible controls, TF vs Histone, FRiP per group)
###############################################################################
if [[ "$RUN_MACS2" -eq 1 ]]; then
  echo "=== STEP 3: MACS2 peak calling (no bamCompare) ==="
  echo "[INFO] MACS2 groups file: $MACS2_GROUPS_FILE"

  if [[ ! -f "$MACS2_GROUPS_FILE" ]]; then
    echo "[WARN] MACS2 groups file not found; skipping MACS2 step."
  else
    ...
    while read -r ip_spec ctrl_spec name type _rest; do
      ...
      echo "[STEP3] MACS2 group: NAME=$name  TYPE=$type_upper"
      echo "        IP:   $ip_file"
      if [[ "$use_control" -eq 1 ]]; then
      ...
      if [[ "$type_upper" == "HISTONE" ]]; then
        macs_cmd+=( --broad --broad-cutoff "$MACS2_BROAD_CUTOFF" )
      fi

      # Run MACS3 using macs env
      "${macs_cmd[@]}"

      narrowPeak_file="${MACS2_DIR}/${name}_peaks.narrowPeak"
      broadPeak_file="${MACS2_DIR}/${name}_peaks.broadPeak"
      filtered_bed="${MACS2_DIR}/${name}_filtered.bed"
      ...
      echo "[STEP3] MACS2 + filtering complete for group: $name"
    ...
    done < "$MACS2_GROUPS_FILE"
  fi

  echo "=== STEP 3 complete ==="
else
  echo "=== STEP 3 skipped (RUN_MACS2=0) ==="
fi
